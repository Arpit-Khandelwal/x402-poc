...
export async function POST(request: NextRequest)
{
    try {
        ...
        console.log(`[DEBUG_POST_CHAT] Cookie 'message_credits': ${cookieStore.get("message_credits")?.value}`);
        console.log(`[DEBUG_POST_CHAT] Parsed credits: ${credits}`);
        console.log(`[DEBUG_POST_CHAT] All Cookies: ${cookieStore.getAll().map(c => c.name).join(', ')}`);
        let paymentResponse;
